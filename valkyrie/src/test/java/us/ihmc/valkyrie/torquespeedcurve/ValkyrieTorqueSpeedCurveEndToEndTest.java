...
public class ValkyrieTorqueSpeedCurveEndToEndTest {
	...
	public void configureRobotModels(boolean disableAnkleJointLimits) {
		configureRobotModels(disableAnkleJointLimits, smallModelSizeScale, smallModelMassScale);
	}

	public void configureRobotModels(boolean disableAnkleJointLimits, double sizeScale, double massScale) {
		...
		smallSimRobotModel.setModelSizeScale(sizeScale);
		smallSimRobotModel.setModelMassScale(massScale);

		smallRealRobotModel.setModelSizeScale(sizeScale);
		smallRealRobotModel.setModelMassScale(massScale);
		...
		double totalMass = TotalMassCalculator
				.computeSubTreeMass(smallSimRobotModel.createFullRobotModel().getRootBody());
	...
	public void testStepUpWithSquareUp() throws Throwable {
		...
		for (double stepHeight : Arrays.asList(9.5, 6.0, 7.25, 8.25)) {
			try {
				testStepUpWithSquareUp(simRobotModel, stepHeight, realRobotModel.getWalkingControllerParameters(),
						getDataOutputFolder(realDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
			try {
				testStepUpWithSquareUp(simRobotModel, stepHeight, simRobotModel.getWalkingControllerParameters(),
						getDataOutputFolder(simDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
			try {
				testStepUpWithSquareUp(smallSimRobotModel, stepHeight,
						smallRealRobotModel.getWalkingControllerParameters(), getDataOutputFolder(smallRealDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
			try {
				testStepUpWithSquareUp(smallSimRobotModel, stepHeight,
						smallSimRobotModel.getWalkingControllerParameters(), getDataOutputFolder(smallSimDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
		}
	...
	public void testStepUpWithSquareUp(DRCRobotModel robotModel, double stepHeightInches,
	...
	public void testStepUpWithoutSquareUp() throws Throwable {
		...
		for (double stepHeight : Arrays.asList(9.5, 6.0, 7.25, 8.25)) {
			try {
				testStepUpWithoutSquareUp(simRobotModel, stepHeight, realRobotModel.getWalkingControllerParameters(),
						getDataOutputFolder(realDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
			try {
				testStepUpWithoutSquareUp(simRobotModel, stepHeight, simRobotModel.getWalkingControllerParameters(),
						getDataOutputFolder(simDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
			...
			try {
				testStepUpWithoutSquareUp(smallSimRobotModel, stepHeight,
						smallRealRobotModel.getWalkingControllerParameters(), getDataOutputFolder(smallRealDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
	...
	public void testStepUpWithoutSquareUp(DRCRobotModel robotModel, double stepHeightInches,
			WalkingControllerParameters walkingControllerParameters, File dataOutputFolder)
			throws SimulationExceededMaximumTimeException {
		showMemoryUsageBeforeTest();
		BambooTools.reportTestStartedMessage(simulationTestingParameters.getShowWindows());
		...
		try {
			...
			stepTo(0.0, stepStart - 0.5 * footLength - 0.15, stepLength, stepWidth, RobotSide.LEFT, heightMap,
					footsteps, false, false);
			RobotSide firstSide = RobotSide.fromByte(footsteps.getFootstepDataList().getLast().getRobotSide())
					.getOppositeSide();
			stepTo(stepStart + 0.5 * footLength + 0.05, xGoal, stepLength, stepWidth, firstSide, heightMap, footsteps,
					false, true);
			setTimings(footsteps, walkingControllerParameters);
	...
	public void testWalkSlope() throws Throwable {
		...
		for (double slopeAngle : new double[] { -30.0, 30.0, 45.0 }) {
			...
			try {
				testWalkSlope(simRobotModel, Math.toRadians(slopeAngle),
						0.5 * walkingControllerParameters.getSteppingParameters().getDefaultStepLength(),
						walkingControllerParameters, getDataOutputFolder(realDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
		}
		...
		for (double slopeAngle : new double[] { -30.0, 30.0, 45.0 }) {
			...
			try {
				testWalkSlope(simRobotModel, Math.toRadians(slopeAngle),
						0.5 * walkingControllerParameters.getSteppingParameters().getDefaultStepLength(),
						walkingControllerParameters, getDataOutputFolder(simDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
		}
		...
		for (double slopeAngle : new double[] { -30.0, 30.0, 45.0 }) {
			WalkingControllerParameters walkingControllerParameters = smallRealRobotModel
					.getWalkingControllerParameters();
			try {
				testWalkSlope(smallSimRobotModel, Math.toRadians(slopeAngle),
						0.5 * walkingControllerParameters.getSteppingParameters().getDefaultStepLength(),
						walkingControllerParameters, getDataOutputFolder(smallRealDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
		}
		...
		for (double slopeAngle : new double[] { -30.0, 30.0, 45.0 }) {
			WalkingControllerParameters walkingControllerParameters = smallSimRobotModel
					.getWalkingControllerParameters();
			try {
				testWalkSlope(smallSimRobotModel, Math.toRadians(slopeAngle),
						0.5 * walkingControllerParameters.getSteppingParameters().getDefaultStepLength(),
						walkingControllerParameters, getDataOutputFolder(smallSimDataFolder));
			} catch (Throwable e) {
				caughtException = e;
			}
		}
	...
	public void testWalkSlope(DRCRobotModel robotModel, double slopeAngle, double stepLength,
			WalkingControllerParameters walkingControllerParameters, File dataOutputFolder)
			throws SimulationExceededMaximumTimeException {
		showMemoryUsageBeforeTest();
		BambooTools.reportTestStartedMessage(simulationTestingParameters.getShowWindows());
		...
		try {
			...
			drcSimulationTestHelper.setInitialSetup(
					initialSetupForSlope(slopeAngle, slope.getTerrainObject3D().getHeightMapIfAvailable()));
			drcSimulationTestHelper.createSimulation("StepUpWithoutSquareUp");
			SimulationConstructionSet scs = drcSimulationTestHelper.getSimulationConstructionSet();
			double cameraX = 2.0;
	...
	public void testUpstairs() throws Throwable {
		...
		try {
			testUpstairs(simRobotModel, 7.0, 3, realRobotModel.getWalkingControllerParameters(),
					getDataOutputFolder(realDataFolder));
		} catch (Throwable e) {
			caughtException = e;
		}
		try {
			testUpstairs(simRobotModel, 9.5, 10, realRobotModel.getWalkingControllerParameters(),
					getDataOutputFolder(realDataFolder));
		} catch (Throwable e) {
			caughtException = e;
		}
		...
		try {
			testUpstairs(simRobotModel, 7.0, 3, simRobotModel.getWalkingControllerParameters(),
					getDataOutputFolder(simDataFolder));
		} catch (Throwable e) {
			caughtException = e;
		}
		try {
			/*
			 * TODO Needs: SwingTrajectoryParameters.useSingularityAvoidanceInSupport() to
			 * be false and disable smoothing in the
			 * CenterOfMassHeightControlState.computeDesiredCoMHeightAcceleration(...)
			 */
			assertFalse(simRobotModel.getWalkingControllerParameters().getSwingTrajectoryParameters()
					.useSingularityAvoidanceInSupport());
			testUpstairs(simRobotModel, 9.5, 10, simRobotModel.getWalkingControllerParameters(),
					getDataOutputFolder(simDataFolder));
		} catch (Throwable e) {
			caughtException = e;
		}
		...
		try {
			testUpstairs(smallSimRobotModel, 7.0, 3, smallRealRobotModel.getWalkingControllerParameters(),
		...
		try {
			testUpstairs(smallSimRobotModel, 7.0, 3, smallSimRobotModel.getWalkingControllerParameters(),
	...
	public void testUpstairs(DRCRobotModel robotModel, double stairStepHeightInches, int numberOfStairSteps,
			WalkingControllerParameters walkingControllerParameters, File dataOutputFolder)
			throws SimulationExceededMaximumTimeException {
		testUpstairs(robotModel, stairStepHeightInches, numberOfStairSteps, walkingControllerParameters,
				dataOutputFolder, true);
	}

	public void testUpstairs(DRCRobotModel robotModel, double stairStepHeightInches, int numberOfStairSteps,
			WalkingControllerParameters walkingControllerParameters, File dataOutputFolder, boolean showGUI)
			throws SimulationExceededMaximumTimeException {
		
		showMemoryUsageBeforeTest();
		if (!showGUI) simulationTestingParameters.setCreateGUI(false);
	...
	private void exportTorqueSpeedCurves(SimulationConstructionSet scs, File dataParentFolder, String dataNameSuffix,
			String info) {
		Robot robot = scs.getRobots()[0];
		TorqueSpeedDataExporterGraphCreator graphCreator = new TorqueSpeedDataExporterGraphCreator(robot,
				scs.getDataBuffer());
		DataExporterExcelWorkbookCreator excelWorkbookCreator = new DataExporterExcelWorkbookCreator(robot,
				scs.getDataBuffer());
		...
		// Crop the Buffer to In/Out. This is important because of how we use the
		// DataBuffer later and we assume that in point is at index=0:
		scs.cropBuffer();
		scs.gotoInPointNow();
		...
		if (simulationTestingParameters.getCreateGUI()) {
			...
			scs.getStandardSimulationGUI().getViewportPanel().getStandardGUIActions()
					.createVideo(new File(dataFolder, tagName + "_Video.mov"));
			System.out.println("done creating video");
		} else {
			System.out.println("Not creating video -- GUI is disabled");
		}
	...
	private static File getDataOutputFolder(String folderName) throws IOException {
		...
		Path path = Paths.get(
				userHome + File.separator + "Valkyrie" + File.separator + "TorqueSpeed" + File.separator + folderName);
		System.out.println("Got dir of " + path);
		FileTools.ensureDirectoryExists(path);
		return path.toFile();
	}

	public void runTestUpstairs(StairTestConfigurator testConfig) throws Throwable, IOException {
		final double EPS = 1.0E-7; // Threshold for double equality test
		
		configureRobotModels(false, testConfig.robotSizeScaleFactor, testConfig.robotMassScaleFactor);
		ValkyrieRobotModel model = (Math.abs(testConfig.robotSizeScaleFactor - 1.0) < EPS 
				&& Math.abs(testConfig.robotMassScaleFactor - 1.0) < EPS) ? simRobotModel : smallSimRobotModel;
		ValkyrieRobotModel parameterModel = (Math.abs(testConfig.robotSizeScaleFactor - 1.0) < EPS 
				&& Math.abs(testConfig.robotMassScaleFactor - 1.0) < EPS) ? realRobotModel : smallRealRobotModel;
		
		testUpstairs(model, testConfig.stepHeight, testConfig.numSteps, parameterModel.getWalkingControllerParameters(),
				getDataOutputFolder(realDataFolder), testConfig.showGui);
	}

	public class StairTestConfigurator {
		public boolean useScaledRobot;
		public boolean skipReports;
		public boolean disableAnkleLimits;
		public double robotSizeScaleFactor;
		public double robotMassScaleFactor;
		boolean showGui;
		int numSteps;
		double stepHeight;

		public StairTestConfigurator() {
			disableAnkleLimits = false; // Whether to enforce ankle limits
			skipReports = false;        // Do not generate reports -- mostly useful while debugging

			/*
			 * Scaling factors. For the "smaller Valkyrie" Sylvain uses: 
			 *   size scale factor = 0.934; // => for size of 1m70 (5'7") 
			 *   mass scale factor = 0.717; // => for total mass of 90kg (200lbs).
			 * If the size scale factor and mass scale factor are both 1.0, no scaling will be performed 
			 */
			robotSizeScaleFactor = 1.0; // How much bigger/smaller to make the robot
			robotMassScaleFactor = 1.0; // How much heavier/lighter to make the robot

			showGui = false;            // Whether to show the GUI. Video generation requires a GUI.
			numSteps = 3;               // Number of steps to ascend
			stepHeight = 7.0;           // Step height in inches
		}
	}

	public static void main(String args[]) {
		ValkyrieTorqueSpeedCurveEndToEndTest tester = new ValkyrieTorqueSpeedCurveEndToEndTest();
		StairTestConfigurator testConfig = tester.new StairTestConfigurator();
		
		// Override any defaults here
		testConfig.showGui = true;
		testConfig.robotSizeScaleFactor = 2.0;
		testConfig.robotMassScaleFactor = 0.5;

		try {
			tester.runTestUpstairs(testConfig);
		} catch (Throwable e) {
